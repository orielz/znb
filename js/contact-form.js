$(function(){
	...
	$("a#submit").click(function(){
		...
		var dataString = 'name=' + $("input#name").val() + '&email=' + email.value + '&phone=' + phone.value + '&message=' + message.value + '&owner_email=' + owner_email + '&stripHTML=' + stripHTML + '&smtpMailServer=' + smtpMailServer;
		...
		$.ajax({
			...
			success: function(){
				$('.error').fadeOut(0);
				$('form#contact-form').clearForm();
				$('#contact_form').html("<div>ההודעה נשלחה!</div>").append("<br><label for='message'><strong>נחזור אליך בהקדם האפשרי.</strong></label>").fadeOut(0).fadeIn(1500, function(){
					$('#contact_form').append("<br><br><a id='back' onclick='window.location.reload(); return false;' class='button'>חזור</a>");
				});
			}
		});				
		return false;
