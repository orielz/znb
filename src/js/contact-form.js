	...
	$("a#submit").click(function(){
		...
		$.ajax({
			...
			success: function(){
				$('.error').fadeOut(0);
				$('form#contact-form').clearForm();
				//$('#contact_form').html("<div>?????? ?????!</div>").append("<br><label for='message'><strong>???? ???? ??? ?????.</strong></label>").fadeOut(0).fadeIn(1500, function(){
				//	$('#contact_form').append("<br><br><a id='back' onclick='window.location.reload(); return false;' class='button'>????</a>");
				//});
				$('#contact_form').hide();
				$('#form-sent').show();
			}
